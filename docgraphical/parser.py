"""DocGraphical Markdown Parsing & Extraction Engine."""
...
def extract_toc(file_path: str, format_type: str = "text") -> str:
    ...
    if not headings:
        return f"[DocGraphical] No Markdown headings found in {os.path.basename(file_path)}"
    ...
    if format_type == "json":
        ...
        lines = [f"=== [DocGraphical TOC] {os.path.basename(file_path)} ==="]
        for h in headings:
            indent = "  " * (h.level - 1)
            hashes = "#" * h.level
...
def extract_section(
    ...
    header_banner = f"<!-- [DocGraphical Section] {os.path.basename(file_path)} | Line {matched_idx + 1} -->\n"
